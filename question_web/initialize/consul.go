...
func InitConsulClient() {
	...
	var err error
	global.ConuslClient, err = consulApi.NewClient(cfg)
	if err != nil {
		panic(err)
	}
...
func Register() {
	...
	err := global.ConuslClient.Agent().ServiceRegister(srv)
	if err != nil {
		zap.S().Error("服务注册失败")
		panic(err)
	}
...
func UnRegister() {
	//服务注销
	err := global.ConuslClient.Agent().ServiceDeregister(global.ServeID)
	if err != nil {
		zap.S().Info("注销失败", err)
	}
	zap.S().Info("注销成功")
